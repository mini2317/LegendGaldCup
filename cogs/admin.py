...
class Admin(commands.Cog):
    ...
    async def set_announce_channel(self, interaction: discord.Interaction, channel: discord.TextChannel):
        ...
        from database import check_and_set_welcome
        
        is_first_time = await check_and_set_welcome(interaction.guild_id)
        ...
        if is_first_time:
            ...
            # 첫 연결이 아닐 경우(단순 채널 변경 등) 짧은 권한 테스트 메시지만 전송
            try:
                test_msg = await channel.send("✅ 채널 설정 및 권한 부여 확인 완료.")
                await test_msg.delete(delay=2) # 2초 후 삭제
            except discord.Forbidden:
                logger.warning(f"Failed to send test message to {channel.id} due to permission issue.")
                return
