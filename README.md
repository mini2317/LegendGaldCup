...
* **`!관리자목록`**: 봇 기능에 접근할 수 있는 관리자들의 멘션 리스트를 조회합니다.
* **`!주제관리` (관리자 전용)**: 언제든 유저들이 건의한 아이디어 주제 목록을 열람하고, 이를 심사하여 진짜 진행 **대기열(Queue)**로 승격시키거나 AI에게 가공을 맡기는 인터페이스를 엽니다.
* **`!대기열관리` (관리자 전용)**: 실제로 다음 차례에 전체 서버에 송출될 **대기열(Queue)** 안의 순서와 현황을 관리, 수정, 삭제하거나 즉시 강제 송출합니다.
* **`!AI주제충전 <개수>` (관리자 전용)**: AI(Gemini)를 호출해 1~5개의 창작 주제를 즉석에서 만들어내어 **대기열(Queue)**에 바로 장전합니다.
